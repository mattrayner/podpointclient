...
def complete_firmware_fixture():
    return load_fixture('complete_firmware')['data'][0]
...
def test_serialisation():
    pod = Pod(data=complete_pod_fixture())
    pod.firmware = Firmware(data=complete_firmware_fixture())
    ...
    assert pod.model.to_json() == '{"id": 123, "name": "S7-UC-03-ACA", "vendor": "Pod Point", "supports_payg": false, "supports_ocpp": false, "supports_contactless": false, "image_url": null}'
    assert pod.location.to_json() == '{"lat": 51.4995, "lng": 0.1248}'
    assert pod.statuses[0].to_json() == '{"id": 2, "name": "Charging", "key_name": "charging", "label": "Charging", "door": "A", "door_id": 1}'
    assert pod.firmware.to_json() == '{"serial_number": "123456789", "version_info": {"manifest_id": "A30P-3.1.22-00001"}, "update_status": {"is_update_available": false}}'
